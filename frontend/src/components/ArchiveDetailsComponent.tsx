...
import { archiveFileStatusQueryOptions } from "../lib/api/queries/archives.queries";
...
export default function ArchiveDetailsComponent({
	...
	const { data: fileStatus } = useQuery(
		archiveFileStatusQueryOptions(String(research.id)),
	);
	const viewResearchUrl = fileStatus?.viewUrl;
		...
		<div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
			...
			<div className="mt-8 rounded-xl bg-white p-6 shadow-sm sm:p-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					{/* PDF Link Section */}
					{fileStatus?.hasFile && viewResearchUrl && (
						<div>
							<p className="text-sm font-medium text-gray-500 mb-2">
								Research Document
							</p>
							<a
								href={viewResearchUrl}
								target="_blank"
